...
fun EditCourse(onClose:(Boolean)-> Unit, course: Course, courseViewModel: CourseViewModel){
    ...
    var rating by remember { mutableStateOf(course.rating)}
    ...
    Column(
        ...
        CustomTextField(
            value = if (startDate==null) course.startDate else convertMillisToDate(startDate),
            readOnly = true,
            onChange = {},
        ...
        CustomTextField(
            value = if (endDate==null) course.endDate else convertMillisToDate(endDate),
            readOnly = true,
            onChange = {},
            trailingIcon = {
                IconButton(
                        ...
                        if(validateInputs()){
                            courseViewModel.editCourse(
                                Course(
                                    ...
                                    if (endDate == null && course.endDate.isNotEmpty()) course.endDate else convertMillisToDate(endDate),
                                    rating
                                )
                            )
                            onClose(true)
